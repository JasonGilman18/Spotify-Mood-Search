...
def loadDataset():
    excel = pandas.read_excel("./frontend/static/data/dataset.xlsx")
...
def createCentriods(ls):
    ...
    for song in ls:
        ...
        if album not in album_centroid:
            album_centroid[album] = {"count": 1, "albumName": song["albumName"], "albumID": song["albumID"], "artistName": song["artistName"], "artistID": song["artistID"], "acousticness": song["acousticness"], "danceability": song["danceability"], "energy": song["energy"], "instrumentalness": song["instrumentalness"], "liveness": song["liveness"], "speechiness": song["speechiness"], "valence": song["valence"]}
        ...
        album_centroid[album] = {"count": album_centroid[album]["count"] + 1, "albumName": song["albumName"], "albumID": song["albumID"], "artistName": song["artistName"], "artistID": song["artistID"], "acousticness": album_centroid[album]["acousticness"] + song["acousticness"], "danceability": album_centroid[album]["danceability"] + song["danceability"], "energy": album_centroid[album]["energy"] + song["energy"], "instrumentalness": album_centroid[album]["instrumentalness"] + song["instrumentalness"], "liveness": album_centroid[album]["liveness"] + song["liveness"], "speechiness": album_centroid[album]["speechiness"] + song["speechiness"], "valence": album_centroid[album]["valence"] + song["valence"]}
    ...
    for album in album_centroid:
        ...
        for key in album_centroid[album].keys():
            if key != "albumName" and key != "albumID" and key != "artistName" and key != "artistID":
